#!/usr/bin/env bash
# check-int-div-grounding.sh — AST-level structural equivalence
# check for the integer-division V1 fmgr cluster.
#
# Sister to check-int-arith-grounding.sh. Pipeline:
#   Lean.Pg.Ir.Emit.IntDivC.main → /tmp/.../lean_emit.c
...
WORK="$(mktemp -d -t pgir-div-grounding.XXXXXX)"
...
(
  ...
  LEAN_PATH=. lean -o Pg/Ir/Emit/IntDivC.olean         Pg/Ir/Emit/IntDivC.lean
)

( cd "$LEAN_DIR" && LEAN_PATH=. lean --run Pg/Ir/Emit/IntDivC.lean ) > "$WORK/lean_emit.c"
...
# int.c covers int2div, int4div.
# int8.c covers int8div.
...
# int.c covers int2div, int4div.
for fn in int2div int4div; do check int "$fn"; done
# int8.c covers int8div.
check int8 int8div

total=$((pass + fail))
if [ "$fail" -eq 0 ]; then
  echo "ok: $pass / $total division functions structurally equivalent to real Postgres."
  exit 0
else
  echo "ERROR: $fail / $total division functions structurally differ from real Postgres:" >&2
  echo "$fails" | tr ' ' '\n' | grep -v '^$' | sed 's/^/  - /' >&2
  exit 1
fi
