#!/usr/bin/env bash
# check-int-arith-grounding.sh — AST-level structural equivalence
# check for the integer-arithmetic V1 fmgr cluster.
#
# Sister to check-int-hash-grounding.sh. Pipeline:
#   Lean.Pg.Ir.Emit.IntArithC.main → /tmp/.../lean_emit.c
...
# (fmgrprotos.h, errcodes.h) — same trick as check-int-cmp-grounding.sh.
LIBPG_INCLUDE=""
for candidate in \
...
WORK="$(mktemp -d -t pgir-arith-grounding.XXXXXX)"
...
(
  ...
  LEAN_PATH=. lean -o Pg/Ir/Emit/IntArithC.olean       Pg/Ir/Emit/IntArithC.lean
)

( cd "$LEAN_DIR" && LEAN_PATH=. lean --run Pg/Ir/Emit/IntArithC.lean ) > "$WORK/lean_emit.c"
...
# int.c covers int2/int4/int24/int42 pl/mi/mul.
# int8.c covers int8/int48/int84/int28/int82 pl/mi/mul.
...
# Same-type and int2/int4-cross-type bodies live in int.c.
for fam in int2 int4 int24 int42; do
  for op in pl mi mul; do check int ${fam}${op}; done
done
# int8 + any cross-type with int8 lives in int8.c.
for fam in int8 int48 int84 int28 int82; do
  for op in pl mi mul; do check int8 ${fam}${op}; done
done

total=$((pass + fail))
if [ "$fail" -eq 0 ]; then
  echo "ok: $pass / $total arithmetic functions structurally equivalent to real Postgres."
  exit 0
else
  echo "ERROR: $fail / $total arithmetic functions structurally differ from real Postgres:" >&2
  echo "$fails" | tr ' ' '\n' | grep -v '^$' | sed 's/^/  - /' >&2
  exit 1
fi
