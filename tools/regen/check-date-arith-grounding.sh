#!/usr/bin/env bash
# check-date-arith-grounding.sh — AST-level structural equivalence check
# for the date-arithmetic V1 fmgr cluster.
#
# Sister to check-int-arith-grounding.sh. Pipeline:
#   Lean.Pg.Ir.Emit.DateArithC.main → /tmp/.../lean_emit.c
#   clang -ast-dump=json on Lean emit AND real PG date.c
...
WORK="$(mktemp -d -t pgir-date-grounding.XXXXXX)"
...
(
  ...
  LEAN_PATH=. lean -o Pg/Ir/Emit/DateArithC.olean       Pg/Ir/Emit/DateArithC.lean
)

( cd "$LEAN_DIR" && LEAN_PATH=. lean --run Pg/Ir/Emit/DateArithC.lean ) > "$WORK/lean_emit.c"
...
# date.c covers date_pli, date_mii, date_mi.
PG_SRC="$PG_INCLUDE/../backend/utils/adt/date.c"
if [ ! -f "$PG_SRC" ]; then
  echo "ERROR: real Postgres date.c not found at $PG_SRC" >&2
  exit 2
fi

clang -Xclang -ast-dump=json -fsyntax-only -I "$PG_INCLUDE" $EXTRA_INC "$PG_SRC" \
  2>/dev/null > "$WORK/real_date.ast.json" || true
if [ ! -s "$WORK/real_date.ast.json" ]; then
  echo "ERROR: clang produced empty AST for date.c" >&2
  exit 2
fi
...
check() {
  local fn=$1
  if "$DIFF_BIN" --left "$WORK/real_date.ast.json" --right "$WORK/lean.ast.json" \
...
total=$((pass + fail))
if [ "$fail" -eq 0 ]; then
  echo "ok: $pass / $total date arithmetic functions structurally equivalent to real Postgres."
  exit 0
else
  echo "ERROR: $fail / $total date arithmetic functions structurally differ from real Postgres:" >&2
  echo "$fails" | tr ' ' '\n' | grep -v '^$' | sed 's/^/  - /' >&2
  exit 1
fi
