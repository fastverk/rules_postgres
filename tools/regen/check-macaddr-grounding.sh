#!/usr/bin/env bash
# check-macaddr-grounding.sh — verify Lean spec ≡ real Postgres C at
# the AST level for the macaddr cluster.
#
# Companion to regen-macaddr.sh (which handles Lean → Rust and Lean → C).
...
#
# Pipeline:
#   Lean.Pg.Ir.Emit.MacaddrC.main → /tmp/.../lean_emit.c
...
WORK="$(mktemp -d -t pgir-macaddr-grounding.XXXXXX)"
...
( cd "$LEAN_DIR" && LEAN_PATH=. lean --run Pg/Ir/Emit/MacaddrC.lean ) > "$LEAN_C"
...
# Locate the real postgres mac.c source.
PG_BACKEND=""
for candidate in \
    /Users/mattmarshall/Library/Caches/bazel/_bazel_mattmarshall/0fee58293a938d2caa5da4f904216d74/external/+postgres_src_extension+postgres_src/src/backend/utils/adt/mac.c \
    /Users/mattmarshall/Library/Caches/bazel/_bazel_mattmarshall/90476f1c3b5e144c1cb932d1d6a2c5b0/external/+pg+postgres_src/src/backend/utils/adt/mac.c; do
  if [ -f "$candidate" ]; then PG_BACKEND="$candidate"; break; fi
done
if [ -z "$PG_BACKEND" ]; then
  echo "ERROR: postgres mac.c not found in bazel cache." >&2
  exit 2
fi
...
# Note: macaddr hash functions (hashmacaddr, hashmacaddrextended) call hash_any/hash_any_extended
# which are defined in src/common/hashfn.c, not mac.c. For v0, we verify only the
# comparison operators which are self-contained within mac.c.
# The hash operators are validated through differential tests in pg_macaddr's cargo tests.
FUNCTIONS=(
  "macaddr_eq"
  "macaddr_ne"
  "macaddr_lt"
...
FUNCTIONS+=("hashmacaddr" "hashmacaddrextended")
...
TOTAL=${#FUNCTIONS[@]}
if [ $FAIL -eq 0 ]; then
  echo "ok: $PASS / $TOTAL macaddr functions structurally equivalent to real Postgres."
  exit 0
else
