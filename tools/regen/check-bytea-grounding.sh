...
WORK="$(mktemp -d -t pgir-bytea-grounding.XXXXXX)"
trap 'rm -rf "$WORK"' EXIT

(
  cd "$LEAN_DIR"
  LEAN_PATH=. lean -o Pg/Ir/Emit/ByteaCommon.olean Pg/Ir/Emit/ByteaCommon.lean
  LEAN_PATH=. lean -o Pg/Ir/Emit/ByteaC.olean      Pg/Ir/Emit/ByteaC.lean
)

( cd "$LEAN_DIR" && LEAN_PATH=. lean --run Pg/Ir/Emit/ByteaC.lean ) > "$WORK/lean_emit.c"
...
pass=0; fail=0; fails=""
# Just byteacat — bytea_catenate is `static` (file-local linkage, AST
# diff would be noisier than valuable for now; cargo test gates the
# behavior).
for fn in byteacat byteaoctetlen byteaeq; do
...
total=$((pass + fail))
if [ "$fail" -eq 0 ]; then
  echo "ok: $pass / $total bytea functions structurally equivalent to real Postgres."
  exit 0
else
  echo "ERROR: $fail / $total bytea functions structurally differ from real Postgres:" >&2
  echo "$fails" | tr ' ' '\n' | grep -v '^$' | sed 's/^/  - /' >&2
  exit 1
fi
