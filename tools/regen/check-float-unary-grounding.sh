#!/usr/bin/env bash
# check-float-unary-grounding.sh — AST structural diff for the float unary
...
WORK="$(mktemp -d -t pgir-float-unary-grounding.XXXXXX)"
trap 'rm -rf "$WORK"' EXIT

(
  cd "$LEAN_DIR"
  LEAN_PATH=. lean -o Pg/Ir/Emit/FloatUnaryCommon.olean Pg/Ir/Emit/FloatUnaryCommon.lean
  LEAN_PATH=. lean -o Pg/Ir/Emit/FloatUnaryC.olean      Pg/Ir/Emit/FloatUnaryC.lean
)

( cd "$LEAN_DIR" && LEAN_PATH=. lean --run Pg/Ir/Emit/FloatUnaryC.lean ) > "$WORK/lean_emit.c"
...
# float unary fns: all four (float4abs, float4um, float8abs, float8um) live in float.c.
...
check float4abs; check float4um
check float8abs; check float8um

total=$((pass + fail))
if [ "$fail" -eq 0 ]; then
  echo "ok: $pass / $total float unary functions structurally equivalent to real Postgres."
  exit 0
else
  echo "ERROR: $fail / $total float unary functions structurally differ from real Postgres:" >&2
  echo "$fails" | tr ' ' '\n' | grep -v '^$' | sed 's/^/  - /' >&2
  exit 1
fi
