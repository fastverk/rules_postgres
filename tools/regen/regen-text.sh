...
OUT_RS="$REPO_ROOT/rust/pg_text/src/lib.rs"
...
(
  ...
  LEAN_PATH=. lean -o Pg/Ir/Emit/Text.olean       Pg/Ir/Emit/Text.lean
)

TMP="$(mktemp -t text.rs.XXXXXX)"
trap 'rm -f "$TMP"' EXIT
( cd "$LEAN_DIR" && LEAN_PATH=. lean --run Pg/Ir/Emit/Text.lean ) > "$TMP"
