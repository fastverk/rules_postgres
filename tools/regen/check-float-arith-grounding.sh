#!/usr/bin/env bash
# check-float-arith-grounding.sh — AST structural diff for the float arithmetic
...
WORK="$(mktemp -d -t pgir-float-arith-grounding.XXXXXX)"
trap 'rm -rf "$WORK"' EXIT

(
  cd "$LEAN_DIR"
  LEAN_PATH=. lean -o Pg/Ir/Emit/FloatArithCommon.olean Pg/Ir/Emit/FloatArithCommon.lean
  LEAN_PATH=. lean -o Pg/Ir/Emit/FloatArithC.olean      Pg/Ir/Emit/FloatArithC.lean
)

( cd "$LEAN_DIR" && LEAN_PATH=. lean --run Pg/Ir/Emit/FloatArithC.lean ) > "$WORK/lean_emit.c"
...
# float arithmetic fns: all eight (float4pl, float4mi, float4mul, float4div,
# float8pl, float8mi, float8mul, float8div) live in float.c.
...
check float4pl;  check float4mi;  check float4mul;  check float4div
check float8pl;  check float8mi;  check float8mul;  check float8div

total=$((pass + fail))
if [ "$fail" -eq 0 ]; then
  echo "ok: $pass / $total float arithmetic functions structurally equivalent to real Postgres."
  exit 0
else
  echo "ERROR: $fail / $total float arithmetic functions structurally differ from real Postgres:" >&2
  echo "$fails" | tr ' ' '\n' | grep -v '^$' | sed 's/^/  - /' >&2
  exit 1
fi
